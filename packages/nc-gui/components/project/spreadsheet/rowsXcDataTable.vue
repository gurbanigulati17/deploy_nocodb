  ...
  <v-container class="h-100 j-excel-container pa-0 ma-0" fluid>
        ...
        <v-menu bottom offset-y>
          <template #activator="{ on }">
            <div style="min-width: 56px" v-on="on">
              <v-icon class="pa-1 pr-0 ml-2" small color="grey">
                mdi-magnify
              </v-icon>
    ...
    >
      <div class="flex-grow-1 h-100" style="overflow-y: auto">
        ...
        >
          <v-skeleton-loader
            v-if="(!dataLoaded && loadingData) || !meta"
            type="table"
          />
          <template
            v-else-if="
              selectedView &&
              (selectedView.type === 'table' || selectedView.show_as === 'grid')
            "
          >
            <xc-grid-view
          ...
          <template v-else-if="isGallery">
            <gallery-view
              ...
              "
              @historyForm="
                ({ rowIndex, rowMeta }) => historyRow(rowIndex, rowMeta)
              "
            />
          </template>
          <template
            v-else-if="selectedView && selectedView.show_as === 'kanban'"
          >
            <kanban-view
              :nodes="nodes"
              :table="table"
      ...
      >
        <v-tooltip bottom>
          <template #activator="{ on }">
            ...
            >
              <v-icon x-small class="mr-2 nc-automations"> mdi-hook </v-icon>
    ...
    >
      <history-form
        v-if="selectedHistoryRowIndex != null && data[selectedHistoryRowIndex]"
        :key="selectedHistoryRowIndex"
        v-model="data[selectedHistoryRowIndex].row"
        :db-alias="nodes.dbAlias"
        :has-many="hasMany"
        :belongs-to="belongsTo"
        :table="table"
        :old-row.sync="data[selectedHistoryRowIndex].oldRow"
        :is-new="data[selectedHistoryRowIndex].rowMeta.new"
        :selected-row-meta="selectedHistoryRowMeta"
        :meta="meta"
        :sql-ui="sqlUi"
        :primary-value-column="primaryValueColumn"
        :api="api"
        ...
        @input="
          showExpandModal = false;
          data[selectedHistoryRowIndex] &&
            data[selectedHistoryRowIndex].rowMeta &&
            delete data[selectedHistoryRowIndex].rowMeta.new;
          loadTableData();
        "
        @commented="reloadComments"
        @loadTableData="loadTableData"
...
import HistoryForm from "@/components/project/spreadsheet/components/historyForm";
...
export default {
  ...
  data: () => ({
    ...
    selectedHistoryRowIndex: null,
    selectedHistoryRowMeta: null,
    navDrawer: true,
    selected: {
      row: null,
      col: null,
  ...
  methods: {
    ...
    loadNext() {
      ...
      this.selectedHistoryRowIndex =
        ++this.selectedHistoryRowIndex % this.data.length;
    },
    loadPrev() {
      this.selectedExpandRowIndex =
        --this.selectedExpandRowIndex === -1
          ...
          : this.selectedExpandRowIndex;
      this.selectedHistoryRowIndex =
        --this.selectedHistoryRowIndex === -1
          ? this.data.length - 1
          : this.selectedHistoryRowIndex;
    },
    checkAndDeleteTable() {
      if (
        ...
        return this.$toast
          .info("Please delete relations before deleting table.")
          .goAway(3000);
      }
    ...
    async createTableIfNewTable() {
      if (this.nodes.newTable && !this.nodes.tableCreated) {
        const columns = this.sqlUi
          .getNewTableColumns()
          .filter((col) => this.nodes.newTable.columns.includes(col.cn));
        ...
        if (rowMeta.new) {
          try {
            ...
            if (
              ...
              pks.length &&
              pks.every(
                (col) => !rowObj[col._cn] && !(col.columnDefault || col.default)
              )
            ) {
              return this.$toast
            ...
            if (
              this.meta.columns.some((col) => {
                return (
                  !col.ai &&
                  col.rqd &&
                  (rowObj[col._cn] === undefined || rowObj[col._cn] === null) &&
                  !col.default
                );
              })
            ) {
            ...
            if (e.response && e.response.data && e.response.data.msg) {
              ...
              this.$toast
                .error(`Failed to save row : ${e.message}`)
                .goAway(3000);
            }
          }
    ...
    async onCellValueChange(col, row, column) {
      ...
      if (rowMeta.new) {
        ...
        try {
          ...
          const newData = await this.api.update(
            id,
            {
          ...
          if (e.response && e.response.data && e.response.data.msg) {
            ...
            this.$toast
              .error(`Failed to update row : ${e.message}`)
              .goAway(3000);
          }
        }
    ...
    async insertNewRow(atEnd = false, expand = false) {
      ...
      if (expand) {
       ...
       // this.expandRow(this.data.length - 1, rowMeta);
        this.historyRow(this.data.length - 1, rowMeta);
      }
      // this.save()
    },
    ...
    async loadTableDataFn() {
      ...
      try {
        if (this.api) {
          const { list, count } = await this.api.paginatedList(
            this.queryParams
          );
          this.count = count;
    ...
    historyRow(row, rowMeta) {
      this.showHistoryModal = true;
      this.selectedHistoryRowIndex = row;
      this.selectedHistoryRowMeta = rowMeta;
    },
