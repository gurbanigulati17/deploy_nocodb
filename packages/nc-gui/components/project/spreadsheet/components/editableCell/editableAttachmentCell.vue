<template>
  ...
  >
    <div
      v-show="(isForm || _isUIAllowed('tableAttachment')) && dragOver"
      class="drop-overlay"
    >
      <div>
    ...
    <div class="d-flex align-center img-container">
        ...
        <v-tooltip bottom>
          <template #activator="{ on }">
            ...
            <v-icon
              ...
              @click="
                showPDFDialog = true;
                getAuditsAndComments(item.title);
                pdfURL = item.url; /*openUrl(item.url || item.data,'_blank')*/
              "
            >
              {{ item.icon }}
            </v-icon>
            ...
            <v-icon
              ...
              @click="
                ...
                getAuditsAndComments(item.title);
                pdfURL = item.url;  */
                openUrl(item.url|| item.data,'_blank')
              "
            >
              mdi-file
            </v-icon>
      ...
      <div
        v-if="isForm || (active && !isPublicGrid)"
      ...
      >
        <v-icon
          ...
          small
          color="primary"
          class="nc-attachment-add-spinner"
        >
        ...
        >
          <v-icon x-small color=""> mdi-plus </v-icon> Attachment
        </v-btn>
        <v-icon
      ...
      <v-card class="h-100 images-modal">
        <v-card-text class="h-100 backgroundColor">
          <div class="d-flex mx-2">
            <v-btn
              v-if="
                (isForm || _isUIAllowed('tableAttachment')) && !isPublicGrid
              "
              small
              class="my-4"
              :loading="uploading"
          ...
          <div class="d-flex flex-wrap h-100">
            <v-container
              fluid
              style="max-height: calc(90vh - 80px); overflow-y: auto"
            >
              <draggable
                v-model="localState"
                class="row"
                ...
                >
                  ...
                  >
                    ...
                    <v-icon
                      color="grey"
                      class="download-icon"
                      @click.stop="downloadItem(item, i)"
                    >
                      mdi-download
                    </v-icon>
                    <div class="pa-2 d-flex align-center" style="height: 200px">
                      ...
                      <v-icon
                        ...
                        @click="
                          ...
                          getAuditsAndComments(item.title);
                          pdfURL =
                            item.url; /*openUrl(item.url || item.data,'_blank')*/
                        "
                      >
                        {{ item.icon }}
                      </v-icon>
          ...
          >
            ...
            >
              ...
              >
                ...
                <div
                  style="width: 90vh; height: calc(100vh - 150px)"
...
export default {
  ...
  watch: {
    value(val, prev) {
      try {
        this.localState =
          (typeof val === "string" && val !== prev ? JSON.parse(val) : val) ||
          [];
      } catch (e) {
        this.localState = [];
      }
  ...
  methods: {
    async saveComment() {
      try {
        ...
        await this.getAuditsAndComments(currentPdf);
      } catch (e) {
        this.$toast.error(e.message).goAway(3000);
      }
    },

    async getAuditsAndComments(pdfTitle) {
      currentPdf = pdfTitle;
      console.log(currentPdf);
      this.loadingLogs = true;
      const data = await this.$store.dispatch("sqlMgr/ActSqlOp", [
        { dbAlias: this.dbAlias },
        "xcModelRowAuditAndCommentList",
        {
          model_id: this.meta.columns
            .filter((c) => c.pk)
            .map((c) => {
    ...
    async onFileSelection() {
      ...
      for (const file of this.$refs.file.files) {
        try {
          ...
          this.$toast
            .error(e.message || "Some internal error occurred")
            .goAway(3000);
