<template>
  ...
  >
    <table v-if="data" class="xc-row-table nc-grid" style="">
      <thead>
              ...
              @updateCol="
                (...args) =>
                  updateCol(
                    ...args,
                    columnObj.bt &&
                      meta.columns.find((c) => c.cn === columnObj.bt.cn),
                    col,
                    row
                  )
              "
...
export default {
  ...
  computed: {
    ...
    availColNames() {
      return (
        (this.availableColumns && this.availableColumns.map((c) => c._cn)) || []
      );
    },
    groupedAggCount() {
      ...
      return this.aggCount
        ? this.aggCount.reduce(
            (o, { model_id, count }) => ({ ...o, [model_id]: count }),
            {}
          )
        : {};
    },
    style() {
      ...
      for (const c of this.availableColumns) {
        const val =
          (this.columnsWidth && this.columnsWidth[c.alias]) ||
          (c.virtual
            ? "200px"
            : (columnStyling[c.uidt] && columnStyling[c.uidt].w) || "150px");
  ...
  methods: {
    ...
    isRequired(_columnObj, rowObj) {
      ...
      return (
        columnObj &&
        columnObj.rqd &&
        (rowObj[columnObj._cn] === undefined ||
          rowObj[columnObj._cn] === null) &&
        !columnObj.default
      );
    },
        ...
        default: {
          ...
          if (e.key && e.key.length === 1) {
            ...
            this.$set(
              ...
              this.availableColumns[this.selected.col]._cn,
              ""
            );
